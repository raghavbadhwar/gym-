...
class BookingService:
    ...
    def get_member_bookings(
        ...
        # Performance optimization: Use contains_eager to eager load the related gym_class.
        # This prevents an N+1 query problem when accessing b.gym_class.name and other
        # class attributes in the subsequent list comprehension, significantly reducing
        # database calls and improving performance.
        query = self.db.query(ClassBooking).options(contains_eager(ClassBooking.gym_class)).join(Class).filter(
            ClassBooking.member_id == member.id
        )
