...
class BookingService:
    ...
    def get_member_bookings(
        ...
        # using the existing join, preventing N+1 queries when accessing b.gym_class later
        query = self.db.query(ClassBooking).join(Class).options(
            contains_eager(ClassBooking.gym_class)
        ).filter(
            ClassBooking.member_id == member.id
        )
