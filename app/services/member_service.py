...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Replaced 5 individual .count() queries with a single .group_by() query.
        # This reduces database roundtrips from 6 down to 2, significantly improving
        # dashboard load time, especially as the members table grows.
        state_counts_query = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Convert results to a dictionary mapped by Enum instance
        stats = {state: count for state, count in state_counts_query if state is not None}

        active = stats.get(MemberState.ACTIVE, 0)
        at_risk = stats.get(MemberState.AT_RISK, 0)
        dormant = stats.get(MemberState.DORMANT, 0)
        churned = stats.get(MemberState.CHURNED, 0)
        new = stats.get(MemberState.NEW, 0)
