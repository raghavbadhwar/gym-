...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Optimize: replace 5 separate count queries with a single group_by query
        state_counts_query = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Let SQLAlchemy handle Enum mapping natively
        stats = {state: count for state, count in state_counts_query}
