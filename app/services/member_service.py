...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Performance optimization: use a single GROUP BY query to eliminate 5 separate DB calls
        # Expecting roughly a ~3-4x performance speedup
        state_counts_raw = (
            self.db.query(Member.current_state, func.count(Member.id))
            .group_by(Member.current_state)
            .all()
        )
        state_counts = dict(state_counts_raw)
