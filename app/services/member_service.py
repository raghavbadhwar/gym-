...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # OPTIMIZATION: Consolidate multiple O(N) .count() roundtrips into a single pass group_by query
        stats = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        # SQLAlchemy returns the actual Enum objects, map counts directly
        state_counts = {state: count for state, count in stats}
