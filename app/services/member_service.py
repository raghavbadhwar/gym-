...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Replacing 5 separate N+1 queries with a single group_by query
        # to fetch all state counts simultaneously, while retaining the separate total query
        # to ensure missing/null states are correctly counted towards the overall total.
        ...
        # SQLAlchemy natively handles Enum mapping, map the raw values directly
        state_counts = {
            state: count
            for state, count in state_counts_query if state is not None
        }

        # Look up using the Enum members themselves
