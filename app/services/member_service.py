...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Performance Optimization: Calculate member stats using a single database query with group_by
        # This prevents 6 separate N+1 count() queries, significantly reducing database latency.
        counts = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        stats = {state: count for state, count in counts}
