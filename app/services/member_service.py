...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Calculate overall total separately to include members with NULL states
        total = self.db.query(Member).count()

        # ⚡ Bolt Optimization: Replaced 5 individual N+1 .count() queries with a single GROUP BY
        # Expected Impact: Reduces database queries from 6 to 2, improving dashboard load time
        state_counts = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Initialize counts safely using raw Enum instances as keys
        counts = {
