...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Optimization: Use a single GROUP BY query instead of multiple count() queries to prevent N+1 query problems.
        state_counts = self.db.query(
            Member.current_state, func.count()
        ).group_by(Member.current_state).all()
        counts_dict = dict(state_counts)
