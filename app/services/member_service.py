...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Performance optimization: batch counts into a single GROUP BY query
        # to avoid N+1 queries when fetching stats.
        state_counts_query = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Map raw enum instances directly to counts to ensure safe lookups
