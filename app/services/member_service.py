...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Single query to replace 6 separate counts
        ...
        for state, count in results:
            if state is not None:
                # Safe handling of Enum objects vs strings from DB
                enum_val = state.value if hasattr(state, 'value') else state
                stats[MemberState(enum_val)] = count
            else:
                stats[None] = count

        # Calculate dynamic total directly from grouping
