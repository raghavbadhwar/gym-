...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Performance optimization: Replace 6 separate `.count()` queries with a single
        # `.group_by(Member.current_state)` query to eliminate redundant database roundtrips.
        stats = self.db.query(Member.current_state, func.count(Member.id)).group_by(Member.current_state).all()

        total = sum(count for _, count in stats)
        active = next((count for state, count in stats if state == MemberState.ACTIVE), 0)
        at_risk = next((count for state, count in stats if state == MemberState.AT_RISK), 0)
        dormant = next((count for state, count in stats if state == MemberState.DORMANT), 0)
        churned = next((count for state, count in stats if state == MemberState.CHURNED), 0)
        new = next((count for state, count in stats if state == MemberState.NEW), 0)
