...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ OPTIMIZATION: Refactored 6 separate .count() queries into a single GROUP BY query.
        # This solves an N+1 query problem, reducing database latency for the dashboard.
        state_counts_query = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        counts_by_state = {state: count for state, count in state_counts_query}
