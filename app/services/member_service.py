...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Replaced 6 separate COUNT queries with a single GROUP BY query.
        # This reduces database roundtrips from 6 to 1, significantly improving the performance
        # of the dashboard overview endpoint, especially under high concurrent load.
        results = self.db.query(Member.current_state, func.count(Member.id)).group_by(Member.current_state).all()
        counts = {state: count for state, count in results}
