...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Bolt optimization: Use a single group by query instead of 6 separate count queries
        counts = self.db.query(Member.current_state, func.count()).group_by(Member.current_state).all()
        counts_dict = {state: count for state, count in counts}
