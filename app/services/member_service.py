...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Optimization: Use a single group_by query instead of 5 separate count queries
        ...
        # Map group_by results using robust fallback pattern
        for state, count in state_counts:
            if state is None:
                continue
            # Handle variations in database driver returns
            state_key = state.name.lower() if hasattr(state, 'name') else str(state).lower()
            if '.' in state_key:  # If str() returns e.g. 'MemberState.ACTIVE'
                state_key = state_key.split('.')[-1]

            if state_key in stats:
                stats[state_key] = count

        # Keep separate total to include NULL/unmapped states correctly
