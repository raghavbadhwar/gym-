...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Use a single query with group_by instead of 6 separate count queries
        results = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Note: SQLAlchemy group_by on an Enum column returns Enum objects as keys,
        # not strings, so we use MemberState.ENUM_NAME to access them.
