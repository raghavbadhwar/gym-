...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Performance optimization: Replace 6 separate COUNT queries with a single aggregation query
        # This prevents N+1 queries when fetching dashboard statistics
        results = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        counts = {k: v for k, v in results}
