...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Optimization: Perform a single database query grouping by current_state
        # to reduce multiple DB roundtrips.
        state_counts_results = self.db.query(
            Member.current_state, func.count()
        ).group_by(Member.current_state).all()

        # Using raw Enum instances as dictionary keys to safely access counts
        state_counts = dict(state_counts_results)
