...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Use single query with GROUP BY for performance (6 queries -> 1 query)
        stats_query = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Convert to dictionary {state_enum: count}
        counts = {state: count for state, count in stats_query}

        # Extract specific counts (default to 0 if not present)
