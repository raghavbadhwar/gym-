...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt: Optimize member stats by replacing 6 separate COUNT queries
        # with a single query using GROUP BY. This significantly reduces database roundtrips.
        stats_query = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # When querying with SQLAlchemy group_by on an Enum column,
        # the returned keys are Python Enum members (e.g., MemberState.ACTIVE),
        # requiring lookup by the Enum object rather than its string value.
        stats_dict = {state: count for state, count in stats_query}

        active = stats_dict.get(MemberState.ACTIVE, 0)
        at_risk = stats_dict.get(MemberState.AT_RISK, 0)
        dormant = stats_dict.get(MemberState.DORMANT, 0)
        churned = stats_dict.get(MemberState.CHURNED, 0)
        new = stats_dict.get(MemberState.NEW, 0)

        total = sum(count for _, count in stats_query)
