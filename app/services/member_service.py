...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Retain separate total query to include members with NULL or unmapped states
        total = self.db.query(Member).count()

        # ⚡ Bolt Optimization: Replace 5 separate N+1 count queries with a single group_by query
        # This reduces database roundtrips from 6 to 2, significantly improving dashboard load time
        state_counts = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Map enum states natively to counts
