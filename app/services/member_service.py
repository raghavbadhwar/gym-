...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # OPTIMIZATION: Replaced 6 separate count queries with a single GROUP BY query.
        # IMPACT: Reduces N+1 database queries to 1 query, significantly improving performance.
        stats_query = self.db.query(Member.current_state, func.count(Member.id)).group_by(Member.current_state).all()
        counts = {state.value: count for state, count in stats_query if state}
