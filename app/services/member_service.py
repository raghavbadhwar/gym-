...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Group by state in a single query instead of 5 separate .count() queries.
        # This reduces database roundtrips from 6 to 2, significantly improving performance.
        state_counts_query = self.db.query(Member.current_state, func.count(Member.id)).group_by(Member.current_state).all()

        # Initialize counts with 0 for all states
        ...
        active_count = counts[MemberState.ACTIVE]
        ...
        return {
            ...
            "retention_rate": round((active_count / total) * 100, 1) if total > 0 else 0
        }
