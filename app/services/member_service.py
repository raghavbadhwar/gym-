...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Group by current_state to avoid 6 separate count queries
        ...
        for state, count in state_counts:
            # Handle potential NULL states or extra states
            if state in stats:
                stats[state] = count
            total += count
        ...
        return {
            "total": total,
            "active": stats[MemberState.ACTIVE],
            "at_risk": stats[MemberState.AT_RISK],
            "dormant": stats[MemberState.DORMANT],
            "churned": stats[MemberState.CHURNED],
            "new": stats[MemberState.NEW],
            "retention_rate": round((stats[MemberState.ACTIVE] / total) * 100, 1) if total > 0 else 0
        }
