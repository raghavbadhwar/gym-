...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Replaced multiple N+1 .count() queries with a single GROUP BY query.
        # This reduces database roundtrips from 6 down to 2, significantly improving dashboard load time.
        total = self.db.query(Member).count()

        state_counts = self.db.query(Member.current_state, func.count(Member.id)).group_by(Member.current_state).all()
        counts_dict = {state: count for state, count in state_counts}
