...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Execute a single query grouping by state
        # instead of making 5 separate .count() queries. Reduces N+1 queries.
        state_counts = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Map the grouped Enum query results to a dictionary
