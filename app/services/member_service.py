...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Optimization: Replaced 6 separate .count() queries with a single .group_by()
        # database aggregation. This significantly reduces database roundtrips and
        # improves performance for dashboard loading.
        ...
        for state, count in state_counts:
            total += count
            if state:
                # Handle enum mapping safely depending on ORM return type
                state_val = state.value if hasattr(state, 'value') else state
                if state_val in stats:
                    stats[state_val] = count
