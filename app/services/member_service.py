...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # We still need the total count to include members where current_state might be NULL
        total = self.db.query(Member).count()

        # Use func.count(Member.id) instead of func.count() to avoid syntax errors in some DBs
        state_counts = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Map raw enum instances to counts for safe lookups
