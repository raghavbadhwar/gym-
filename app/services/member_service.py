...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Performance optimization: Reduces 6 separate count queries to 2 queries.
        # Calculate total separately to correctly include records with NULL states
        total = self.db.query(Member).count()

        # Single grouped query for all state counts
        state_counts_query = self.db.query(Member.current_state, func.count(Member.id)).group_by(Member.current_state).all()
        # Use Enum instance keys directly from query
