...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt: Grouped query optimization reduces the number of database queries from 6 to 2, improving response time.
        total = self.db.query(Member).count()

        # Group by state to get all counts in a single query
        state_counts = self.db.query(Member.current_state, func.count(Member.id)) \
            .group_by(Member.current_state) \
            .all()
