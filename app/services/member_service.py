...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Optimized: Replace 6 separate count queries with a single group_by query
        # to avoid redundant database roundtrips.
        ...
        for state, count in state_counts:
            # Handle potential None/NULL states safely
            if state is None:
                continue

            # Safely map database result to Enum (handles raw strings or Enum objects)
            state_val = state.value if hasattr(state, 'value') else state

            # Find matching enum key
            enum_key = None
            ...
            if enum_key and enum_key in counts:
