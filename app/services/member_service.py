...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # ⚡ Bolt Optimization: Group by member state to reduce database queries.
        # Previously, there were 6 individual SELECT COUNT(*) queries, causing N+1 query performance issues.
        # This refactor reduces the DB calls to just 2 queries, significantly decreasing execution time
        # and reducing database load.

        # Calculate total members separately to ensure we count all records, including any potential NULL states
        total = self.db.query(Member).count()

        # Execute a single GROUP BY query for all member states
        state_counts_query = self.db.query(
            Member.current_state,
            func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Map results to dictionary for easy access
