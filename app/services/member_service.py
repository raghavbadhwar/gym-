...
class MemberService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Calculate total members independently to avoid excluding NULL or unmapped states
        total = self.db.query(Member).count()

        # ⚡ Bolt Optimization: Replace 5 N+1 queries with a single GROUP BY query
        # This significantly reduces DB roundtrips and memory overhead when fetching stats
        state_counts = self.db.query(
            Member.current_state, func.count(Member.id)
        ).group_by(Member.current_state).all()

        # Map enum to count using native enum instances to prevent fragile string parsing
