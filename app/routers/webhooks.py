...
async def verify_webhook(
    ...
    if hub_mode == "subscribe" and hub_verify_token is not None and settings.whatsapp_verify_token is not None:
        # Security: Encode strings to utf-8 bytes before comparison to prevent TypeError exceptions
        # caused by non-ASCII characters, which could otherwise act as a Denial of Service (DoS) vulnerability.
