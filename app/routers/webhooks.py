...
async def verify_webhook(
    ...
    # SECURITY: Use hmac.compare_digest to prevent timing attacks during token verification
    if hub_mode == "subscribe" and hub_verify_token is not None and settings.whatsapp_verify_token is not None and hmac.compare_digest(hub_verify_token, settings.whatsapp_verify_token):
...
async def receive_message(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # SECURITY: Explicitly catch and re-raise HTTP exceptions to prevent fail-open vulnerability
        # where HTTP errors might be masked as 200 OK responses to Meta
        raise
    except Exception as e:
