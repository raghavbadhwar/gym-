...
async def verify_webhook(
    ...
    if hub_mode == "subscribe" and hub_verify_token is not None and settings.whatsapp_verify_token is not None:
        # Encode arguments to utf-8 bytes to prevent TypeError exceptions caused by non-ASCII characters, which can otherwise act as a Denial of Service (DoS) vulnerability.
