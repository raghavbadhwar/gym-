...
class Conversation(Base):
    ...
    # Add composite index for efficient filtering by member and sorting by time
    __table_args__ = (
        Index("idx_conversation_member_created", "member_id", "created_at"),
    )

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    # Remove individual index on member_id as it's covered by the composite index
    member_id = Column(String(36), ForeignKey("members.id"), nullable=False, index=False)
